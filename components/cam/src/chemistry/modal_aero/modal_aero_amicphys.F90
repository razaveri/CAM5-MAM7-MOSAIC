...
! to save aerosol pH to history file (dsj+zlu)
!!!!!!!!!RAHUL's comment!!!!!!!!!!!!!!!
!Then make history output of the following variables for every mode:
!1)      fhplus(:) = 1 for mc(jc_h,:) > 0 and fhplus(:) = 0 for mc(jc_h,:) = 0
!2)      water_a(:)
!3)      mhplus(:) = mc(jc_h,:)
!4)      pH_output(:) = -log10(mc(jc_h,ibin) for mc(jc_h,:) > 0 and pH_output(:) = 14 for mc(jc_h,:) = 0

!From the monthly averages of the above variables that are automatically computed from the history output, we can compute the real averages as follows:
!1)      water_a_real = water_a/fhplus
!2)      mhplus_real  = mhplus/fhplus
!3)      pH_real_arith_mean = -log10(mhplus_real)   (i.e., based on arithmetic mean of mhplus)
!4)      pH_real_geom_mean = (pH_output -  14*(1 – fhplus))/fhplus  (i.e., based on geometric mean of mhplus)
!!!!Dick's email!!!!!!!!!!!!!!!!!!!!!!!
!Instead I suggest doing outfld calls for the following (for imode = 1:7)).!     1) fhplus(imode)
!     2) water_a_valid(imode) = water_a(imode)*fhplus(imode)
!     3) hplus_valid(imode) = mc(jc_h,imode)*fhplus(imode)
!     4) ph__valid(imode) = [ ph_output(imode) as described above ] * fhplus(imode)
!Whenever H+ is zero, you want all 4 values sent to outfld to be zero.
      ...
      real(r8) :: Hconc_sav(ntot_amode)
      real(r8) :: awater(ntot_amode)
...
!         misc_vars_aa_sub(jsub)                      )
          misc_vars_aa_sub(jsub), Hconc_sav , awater  )
      else
...
! to save aerosol pH (dsj+zlu)
!         misc_vars_aa_sub                            )
          misc_vars_aa_sub, Hconc_sav, awater        )
!
! calculates changes to gas and aerosol sub-area TMRs (tracer mixing ratios)
...
!---------------------------------------------------------------------
 ...
 !              uptkrate_h2so4,    misc_vars_aa_sub                        )
        ...
        use module_data_mosaic_aero,   only: naer_mosaic => naer, &
             ...
             mosaic_vars_aa_type
        use modal_aero_data,           only:mosaic_on_nitrate_off
#ifdef SPMD
...
!       it_mosaic   = nstep
        ...
        do imode = 1, n_mode
           ...
           if (.not.mosaic_on_nitrate_off) then
           if (iaer_no3 > 0) &
...
use modal_aero_data, only : &
    cnst_name_cw, &
    modeptr_accum, modeptr_aitken, modeptr_pcarbon, modeptr_ufine,ntot_amode !dsj+zlu add atot_amode
