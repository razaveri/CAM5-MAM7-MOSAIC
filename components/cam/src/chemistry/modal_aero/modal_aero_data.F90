...
!--------------------------------------------------------------
      ...
      ! this turns modal_aero_amicphys on/off -- default for running at pnnl is on
      ! it should be a namelist variable with default value = 0
      integer, public, protected :: mam_amicphys_optaa=1

      ! when mosaic_aqchem_optaa <= 0, aqueous chem calcs do not affect hclg, cl_ax, no3_ax, and co3_ax species
      integer, public, protected :: mosaic_aqchem_optaa = 1
      ...
      logical, public, protected :: mosaic_on_nitrate_off = .false. !Turn on/off nitrate calculations when mosaic in on (set via namelist)
      logical, public, protected :: idohist_3d_condevap   = .false. !Turn on/off history output for mosaic (set via namelist
...
!--------------------------------------------------------------
!--------------------------------------------------------------
    subroutine modal_aero_data_init( pbuf2d, modal_mosaic_nitrates, mosaic_on_nitrate_off_in, idohist_3d_condevap_in)

       type(physics_buffer_desc), pointer :: pbuf2d(:,:)
       logical, intent(in) :: modal_mosaic_nitrates
       logical, intent(in) :: mosaic_on_nitrate_off_in, idohist_3d_condevap_in
       ...
       ! turn on modal_aero_amicphys when doing mosaic aerosol chemistry
       if ( modal_mosaic_nitrates ) then
          if ( mam_amicphys_optaa <= 0 ) mam_amicphys_optaa = 1
       endif

       mosaic_on_nitrate_off = mosaic_on_nitrate_off_in
       idohist_3d_condevap   = idohist_3d_condevap_in
...
#if ( defined MOSAIC_SPECIES )
! the following code is a temporary means of setting the specmw_amode of the new mosaic species
! the correct way (??) is to set the spec_type of these species (in mode_defs of rad_cnst_nl)
!    to nitrate, sodium, chloride, carbonate, calcium,
!    then modify the case statement above to include these species
! however, spec_type is used in several other modules that may require some changes 
!    (rad_constituents, microp_aero, nucleate_ice_cam, aero_model, modal_aero_deposition)
! somewhat related to this are radiation diagnostics where aod, etc. are calculated 
!    for different species types.  That code should be changed so that seasalt
!    includes sodium, chloride, and the sea-salt fraction of sulfate;
!    and dust should (perhaps) include calcium and carbonate.
                if      (cnst_name(qArrIndex)(1:4) == 'no3_') then
                   specmw_amode(l,m) = 62.0_r8
                else if (cnst_name(qArrIndex)(1:4) == 'ncl_') then  ! ncl is Na instead of NaCl
                   specmw_amode(l,m) = 23.0_r8
                else if (cnst_name(qArrIndex)(1:3) == 'cl_') then
                   specmw_amode(l,m) = 35.5_r8
                else if (cnst_name(qArrIndex)(1:4) == 'co3_') then
                   specmw_amode(l,m) = 60.0_r8
                else if (cnst_name(qArrIndex)(1:3) == 'ca_') then
                   specmw_amode(l,m) = 40.0_r8
                end if
#endif
             endif
 
             if(masterproc) then
                write(iulog,9212) ' name, stype : ', cnst_name(qArrIndex), spec_type
...
!                call endrun( trim(msg) )
       ...
       write(iulog,'(3x,a,2x,i12,2x,a,20x,a)') '-', l, cnst_name(l), 'lptr_h2so4_g_amode'
       ...
       l = lptr_nh3_g_amode
       write(iulog,'(3x,a,2x,i12,2x,a,20x,a)') '-', l, cnst_name(l), 'lptr_nh3_g_amode'
       ...
       l = lptr_hno3_g_amode
       write(iulog,'(3x,a,2x,i12,2x,a,20x,a)') '-', l, cnst_name(l), 'lptr_hno3_g_amode'
       ...
       do i = 1, nsoa
          ...
          write(iulog,'(3x,a,2x,i12,2x,a,20x,a,i2.2,a)') '-', l, cnst_name(l), 'lptr2_soa', i, '_g_amode'
       end do
       ...
       l = lptr_hcl_g_amode
       write(iulog,'(3x,a,2x,i12,2x,a,20x,a)') '-', l, cnst_name(l), 'lptr_hcl_g_amode'
