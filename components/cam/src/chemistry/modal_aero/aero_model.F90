...
module aero_model
 ...
 ! Misc private data 
  ...
  logical :: idohist_3d_condevap   = .false.
  logical :: mosaic_on_nitrate_off = .false.
...
contains
  ...
  subroutine aero_model_readnl(nlfile)
    ...
    namelist /aerosol_nl/ aer_wetdep_list, aer_drydep_list, sol_facti_cloud_borne, &
       ...
       modal_mosaic_nitrates, mosaic_on_nitrate_off, idohist_3d_condevap
    ...
    call mpibcast(mosaic_on_nitrate_off, 1,                         mpilog,  0, mpicom)
    call mpibcast(idohist_3d_condevap,   1,                         mpilog,  0, mpicom)
#endif
  ...
  subroutine aero_model_init( loffset, pbuf2d )
    ...
    use modal_aero_data, only: cnst_name_cw, mam_amicphys_optaa
...
!   logical  :: history_aerosol ! Output MAM or SECT aerosol tendencies
    ...
    call modal_aero_data_init( pbuf2d, modal_mosaic_nitrates, mosaic_on_nitrate_off, idohist_3d_condevap)
    ...
    if ( mam_amicphys_optaa <= 0 ) then
       ...
       call modal_aero_newnuc_init( .false. )
    else
       call modal_aero_calcsize_init( pbuf2d )
       call modal_aero_newnuc_init( .true. )
...
! 3d precip production and evaporation diagnostics
    if (idohist_3d_condevap) then
       field_name = 'conds3d'
       call addfld( field_name, (/ 'lev' /), 'A', 'kg/kg/s', 'stratiform 3d precip production rate')
       if ( history_aerosol ) call add_default( field_name, 1, ' ' )
       field_name = 'evaps3d'
       call addfld( field_name, (/ 'lev' /), 'A', 'kg/kg/s', 'stratiform 3d precip evaporation rate')
       if ( history_aerosol ) call add_default( field_name, 1, ' ' )

       field_name = 'condc3d'
       call addfld( field_name, (/ 'lev' /), 'A', 'kg/kg/s', 'shal-conv 3d precip production rate')
       if ( history_aerosol ) call add_default( field_name, 1, ' ' )
       field_name = 'evapc3d'
       call addfld( field_name, (/ 'lev' /), 'A', 'kg/kg/s', 'shal-conv 3d precip evaporation rate')
       if ( history_aerosol ) call add_default( field_name, 1, ' ' )

       field_name = 'condd3d'
       call addfld( field_name, (/ 'lev' /), 'A', 'kg/kg/s', 'deep-conv 3d precip production rate')
       if ( history_aerosol ) call add_default( field_name, 1, ' ' )
       field_name = 'evapd3d'
       call addfld( field_name, (/ 'lev' /), 'A', 'kg/kg/s', 'deep-conv 3d precip evaporation rate')
       if ( history_aerosol ) call add_default( field_name, 1, ' ' )

! instead add PRECSH, PRECZ, PRECCDZM to fincl1
!      if ( history_aerosol ) call add_default( 'PRECSH',   1, ' ' )
!      if ( history_aerosol ) call add_default( 'PRECZ',    1, ' ' )
!      if ( history_aerosol ) call add_default( 'PRECCDZM', 1, ' ' )
    end if
...
!         if (lphase == 1) then
  ...
  subroutine aero_model_wetdep( state, dt, dlf, cam_out, ptend, pbuf)
    ...
    call wetdep_inputs_set( state, pbuf, dep_inputs, idohist_3d_condevap )
  ...
  subroutine aero_model_gasaerexch( loffset, ncol, lchnk, troplev, delt, reaction_rates, &
    ...
    use modal_aero_data,       only : cnst_name_cw, qqcw_get_field, mam_amicphys_optaa
#if ( defined ( MOSAIC_SPECIES ) )
...
! do gas-aerosol exchange (h2so4, msa, nh3 condensation)
    ...
    mam_amicphys: if ( mam_amicphys_optaa > 0 ) then
