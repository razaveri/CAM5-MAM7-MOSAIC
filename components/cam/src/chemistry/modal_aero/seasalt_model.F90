...
contains
  ...
  real(r8), parameter :: fracemit_cl =  0.5375583_r8        
...
iso4= so4_indices(ibin)
 ...
 cflx(:ncol,iso4)=0.0_r8 !zlu non-zero?
#endif
       ...
       do i=1, nsections
        ...
        cflx(:ncol,mm)=cflx(:ncol,mm)+fi(:ncol,i)*ocnfrc(:ncol)*emis_scale  &   !++ ag: scale sea-salt
                  *4._r8/3._r8*pi*rdry(i)**3*dns_aer_sst* fracemit_na   ! should use dry size, convert from number to mass flux (kg/m2/s)
       cflx(:ncol,icl)=cflx(:ncol,icl)+fi(:ncol,i)*ocnfrc(:ncol)*emis_scale   &   !++ ag: scale sea-salt
                  *4._r8/3._r8*pi*rdry(i)**3*dns_aer_sst* fracemit_cl  ! should use dry size, convert from number to mass flux (kg/m2/s)
       cflx(:ncol,iso4)=cflx(:ncol,iso4)+fi(:ncol,i)*ocnfrc(:ncol)*emis_scale  &   !++ ag: scale sea-salt
                  *4._r8/3._r8*pi*rdry(i)**3*dns_aer_sst* fracemit_so4 ! should use dry size, convert from number to mass flux (kg/m2/s)
