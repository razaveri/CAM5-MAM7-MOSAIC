...
module MO_SETSOX
  ...
  integer :: id_hcl
#endif
#if (defined MODAL_AERO_BOX_MODEL)
  public ::  id_msa, id_so2, id_nh3, id_hno3, id_h2o2, id_o3, id_ho2, id_h2so4
#if ( defined MOSAIC_SPECIES )
  public ::  id_hcl
#endif
#endif
...
!-----------------------------------------------------------------------      
  subroutine SETSOX( &
    ...
    use modal_aero_data, only : mosaic_aqchem_optaa, mosaic_on_nitrate_off
...
#if (defined MODAL_AERO_BOX_MODEL)
    real(r8),         intent(out), optional :: yph_out(:,:)         ! ph value
#endif
    ...
    real(r8), parameter :: xl_cutoff = 1.e-8_r8
...
ver_loop_init: &
    do k = 1,pver
       ...
       if (id_hno3 > 0) then
          ...
          if (mosaic_on_nitrate_off) xhno3(:,k) = 0.0_r8
       else
          xhno3(:,k) = 0.0_r8
       endif
    ...
    ver_loop0: do k = 1,pver                               !! pver loop for STEP 0
       col_loop0: do i = 1,ncol
          ...
          if (cloud_borne .and. cldfrc(i,k)>0._r8) then
             ...
             if (mosaic_on_nitrate_off) xno3(i,k) = 0.0_r8
#if ( defined MOSAIC_SPECIES )
          ...
          xl_test0: if( xl >= xl_cutoff ) then
             ...
             ! equivalent new code including mosaic species
             !    hehno3 = xk + xk*xe/hplus                      [(mol-N/ltr-W) / atm-N]
             !    px = hehno3(i,k) * Ra * tz * xl = no3/hno3g    [dimensionless]
             !    hno3g = (xhno3+xno3)/(1 + px)                  [in mol-N/mol-A]
             !          = (xhno3+xno3)/(1 + hehno3*ra*tz*xl)
             !          = (xhno3+xno3)/(1 + xk*ra*tz*xl*(1 + xe/hplus)
             !    ehno3 = hno3g*xk*xe*patm
             !          = xk*xe*patm*(xhno3+xno3)/(1 + xk*ra*tz*xl*(1 + xe/hplus)
             !          = ( fact1_hno3    )/(1 + fact2_hno3 *(1 + fact3_hno3/hplus)
             !    [no3-] = ehno3/hplus
             ! *** note that xno3 is non-zero only when mosaic_species is defined
         ...
         ! xk and xe from CAPRAM 2.4 model = from Marsh and McElroy (1985, Atmos Environ)
             ...
             !    px = heso2(i,k) * Ra * tz * xl = sivc/so2g  [in mol/mol]
             !    so2g = xso2/(1 + px)
             ...
             !        ... co2 effects
             !-----------------------------------------------------------------
             ! Eco2 / hplus = mol-HCO3/ltr-W
             co2g = 330.e-6_r8                            !330 ppm = 330.e-6 mol/mol
             ...
             !         ... so4 effect
             !-----------------------------------------------------------------
             ! Eso4(mol-S/ltr-W) = xso4(mol-S/mol-A) * xhnm(molec-A/cm3-A) * const0[(cm3-A/ltr-A)/(molec-A/mol-A)] / xl(ltr-W/ltr-A)
             ...
             iter_loop0: do iter = 1,itermax
                ...
                if (mosaic_on_nitrate_off) tmp_no3  = 0.0_r8
...
#if (defined MODAL_AERO_BOX_MODEL)
             if ( present( yph_out ) ) then
                yph_out(i,k) = yph
             else
                qin(i,k,gas_pcnst) = yph
             end if
#endif
    ...
    ver_loop1: do k = 1,pver
      ...
      ! xk and xe from CAPRAM 2.4 model - from Marsh and McElroy (1985, Atmos Environ)
          ...
          if (mosaic_on_nitrate_off) hno3g(i,k) = 0.0_r8
          ...
          xl_test1: IF (XL .ge. xl_cutoff) THEN    !! WHEN CLOUD IS PRESENTED          
          ...
          END IF xl_test1 !! WHEN CLOUD IS PRESENTED
