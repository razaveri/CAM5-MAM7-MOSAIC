...
!==============================================================================
subroutine wetdep_inputs_set( state, pbuf, inputs, idohist_3d_condevap )
  ...
  logical, optional :: idohist_3d_condevap
  ...
  if ( present( idohist_3d_condevap ) ) then
     if ( idohist_3d_condevap) then
        q_tmp(:,:) = 0.0_r8
        q_tmp(1:ncol,:) = inputs%prain(1:ncol,:)
        call outfld( 'conds3d', q_tmp, pcols, state%lchnk )
        q_tmp(1:ncol,:) = inputs%evapr(1:ncol,:)
        call outfld( 'evaps3d', q_tmp, pcols, state%lchnk )
        q_tmp(1:ncol,:) = rprdsh(1:ncol,:)
        call outfld( 'condc3d', q_tmp, pcols, state%lchnk )
        q_tmp(1:ncol,:) = evapcsh(1:ncol,:)
        call outfld( 'evapc3d', q_tmp, pcols, state%lchnk )
        q_tmp(1:ncol,:) = rprddp(1:ncol,:)
        call outfld( 'condd3d', q_tmp, pcols, state%lchnk )
        q_tmp(1:ncol,:) = evapcdp(1:ncol,:)
        call outfld( 'evapd3d', q_tmp, pcols, state%lchnk )
     end if
  end if
