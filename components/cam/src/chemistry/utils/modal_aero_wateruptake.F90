...
use modal_aero_data, only:  dgnumlo_amode, &
         numptr_amode, ntot_amode, nsoa, npoa, nbc, &
         ...
         lptr2_soa_a_amode,  lptr2_bc_a_amode,  lptr2_pom_a_amode, &
         lptr2_soa_g_amode
use modal_aero_amicphys, only:  hygro_bc, hygro_pom, hygro_soa, hygro_dst
#endif
...
!  real(r8), pointer :: q(:,:,:), gas(:)    ! tracer array 
! dick comment to steve - it was not clear why gas was a pointer here
! and I did not see a statement with "gas => ..."
   real(r8), pointer :: q(:,:,:)
...
!   allocate( gas(ngas_volatile) )
   ...
   do i = 1, ncol
      ...
      do m = 1, ntot_amode
         ...
         lq = lptr2_bc_a_amode(m,1)
         if (1 <= lq .and. lq <= pcnst) &
         aer(ibc_a,   jtotal, m) = q(i,k,lq)*factaermass

         lq = lptr2_pom_a_amode(m,1)
         if (1 <= lq .and. lq <= pcnst) &
