...
subroutine modal_aer_opt_init()
   ...
   call addfld ('AODCL',         horiz_only, 'A','  ',    'Aerosol optical depth 550 nm from nss chlorine',  flag_xyfill=.true.)
   call addfld ('AODSO4',        horiz_only, 'A','  ',    'Aerosol optical depth 550 nm from nss SO4',       flag_xyfill=.true.)
#else
...
#if ( defined MOSAIC_SPECIES )
   call addfld ('BURDENCL',      horiz_only, 'A','kg/m2', 'Non-seasalt chlorine aerosol burden' ,            flag_xyfill=.true.)
   call addfld ('BURDENSO4',     horiz_only, 'A','kg/m2', 'Non-seasalt sulfate aerosol burden' ,             flag_xyfill=.true.)
#else
...
subroutine modal_aero_sw(list_idx, state, pbuf, nnite, idxnite, &
   ...
   real(r8) :: burdendust(pcols), burdenso4(pcols), burdenbc(pcols), burdenno3(pcols), burdennh4(pcols), &
   ...
   real(r8) :: scatdust(pcols), scatso4(pcols), scatno3(pcols), scatnh4(pcols), scatbc(pcols), &
               scatpom(pcols), scatsoa(pcols), scatseasalt(pcols)
   real(r8) :: absdust(pcols), absso4(pcols), absno3(pcols), absnh4(pcols), absbc(pcols), &
               abspom(pcols), abssoa(pcols), absseasalt(pcols)
   real(r8) :: hygrodust(pcols), hygroso4(pcols), hygrono3(pcols), hygronh4(pcols), hygrobc(pcols), &
   ...
   ! total species AOD
   real(r8) :: dustaod(pcols), so4aod(pcols), no3aod(pcols), nh4aod(pcols), claod(pcols), bcaod(pcols), &
...
#if ( defined MOSAIC_SPECIES )
   real(r8) ::  ssso4(pcols),nssso4(pcols),so4(pcols),seasalt(pcols)
   real(r8) ::  sscl(pcols),nsscl(pcols),cl(pcols),na(pcols)
   real(r8) ::  volcl(pcols),volso4(pcols),volss(pcols),volna(pcols)
   real(r8) ::  dens_so4,dens_ss,dens_cl
   real(r8) ::  specrefr_so4,specrefi_so4,hygro_so4
   real(r8) ::  specrefr_ss,specrefi_ss,hygro_ss
   real(r8) ::  specrefr_cl,specrefi_cl,hygro_cl
   real(r8) ::  burdencl(pcols),scatcl(pcols),abscl(pcols),hygrocl(pcols)
#endif
   ...
   do m = 1, nmodes
      ...
      do isw = 1, nswbands
         ...
         do k = top_lev, pver
            ...
            hygrocl(:ncol)      = 0._r8
! rce 2017-08-12
! some modes to not have the ncl species (=na with mosaic-on)
!    but they can have some nss-so4 and nss-cl
! these variables need to be set to zero for those modes because the 
!    "if (trim(spectype) == 'seasalt') then" block does not execute for them
            na(:ncol)           = 0._r8
            cl(:ncol)           = 0._r8
            sscl(:ncol)         = 0._r8
            so4(:ncol)          = 0._r8
            ...
            do l = 1, nspec
               ...
               if (savaervis) then
                  ...
                  if (trim(spectype) == 'sulfate') then
                     dens_so4=specdens
                     specrefr_so4=specrefr
                     specrefi_so4=specrefi
                     hygro_so4=hygro_aer
                     do i = 1, ncol
                        so4(i) = specmmr(i,k)
                        volso4(i)=vol(i)
                  ...
                  if (trim(spectype) == 'seasalt') then
                     dens_ss=specdens
                     specrefr_ss=specrefr
                     specrefi_ss=specrefi
                     hygro_ss=hygro_aer
                     do i = 1, ncol
                        na(i)=specmmr(i,k)
                        volna(i)=vol(i)
                  ...
                  if (trim(spectype) == 'chloride') then
                     dens_cl=specdens
                     specrefr_cl=specrefr
                     specrefi_cl=specrefi
                     hygro_cl=hygro_aer
                     do i = 1, ncol
                        cl(i) = specmmr(i,k)
                        volcl(i)=vol(i)
...
#if ( defined MOSAIC_SPECIES )
! rce 2017-08-12 - the added checks for (mass > 0.0_r8) are to
!    avoid problems with modes that do not contain one or more of ncl(=na), cl, or so4
            do i = 1, ncol 
               ssso4(i)=min(ssso4(i),so4(i))
               nssso4(i)=so4(i)-ssso4(i)
               if (nssso4(i) .gt. 0.0_r8) then  ! rce 2017-08-12
               volso4(i)=max( 0.0_r8, volso4(i)-ssso4(i)/dens_so4 )    ! non-seasalt so4 volume
               burdenso4(i) = burdenso4(i) + nssso4(i)*mass(i,k)
               scatso4(i)   = volso4(i)*specrefr_so4
               absso4(i)    = -volso4(i)*specrefi_so4
               hygroso4(i)  = volso4(i)*hygro_so4
               end if
               
               sscl(i)=min(sscl(i),cl(i))
               nsscl(i)=cl(i)-sscl(i)
               if (nsscl(i) .gt. 0.0_r8) then  ! rce 2017-08-12
               volcl(i)=max( 0.0_r8, volcl(i)-sscl(i)/dens_cl )    ! non-seasalt cl volume
               burdencl(i) = burdencl(i) + nsscl(i)*mass(i,k)
               scatcl(i)   = volcl(i)*specrefr_cl
               abscl(i)    = -volcl(i)*specrefi_cl
               hygrocl(i)  = volcl(i)*hygro_cl
               end if
               
               if (na(i) .gt. 0.0_r8) then  ! rce 2017-08-12
               seasalt(i)=ssso4(i)+sscl(i)+na(i)
               volss(i)=volna(i)+ssso4(i)/dens_so4+sscl(i)/dens_cl  
               burdenseasalt(i) = burdenseasalt(i) + seasalt(i)*mass(i,k)
               scatseasalt(i)   = volss(i)*specrefr_ss
               absseasalt(i)    = -volss(i)*specrefi_ss
               hygroseasalt(i)  = volss(i)*hygro_ss
               end if
            end do
#endif
            ...
            if (savaervis) then
		     ...
		     sumscat        = scatso4(i) + scatno3(i) + scatnh4(i) + scatpom(i) + scatsoa(i) + scatbc(i) + &
                                      scatdust(i) + scatseasalt(i) + scath2o
		     sumabs         = absso4(i) + absno3(i) + absnh4(i) + abspom(i) + abssoa(i) + absbc(i) + &
                                      absdust(i) + absseasalt(i) + absh2o
                     sumhygro       = hygroso4(i) + hygrono3(i) + hygronh4(i) + hygropom(i) + hygrosoa(i) + hygrobc(i) + &
                     ...
                     aodc           = (abscl(i)*(1.0_r8 - palb(i)) + palb(i)*scatcl(i))*dopaer(i)
                     claod(i)      = claod(i) + aodc
#endif
...
!                  if (nerr_dopaer >= nerrmax_dopaer) then
   ...
   if (list_idx == 0) then
      ...
      do i = 1, nnite
         ...
         burdenno3(idxnite(i))  = fillvalue
         burdennh4(idxnite(i))  = fillvalue
